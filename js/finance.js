...
async function signIn() { if (!firebaseConfigured) { showMessage('loginMessage', 'firebase.js дотор Firebase config-оо оруулна уу.'); return; } try { await signInWithPopup(auth, googleProvider); } catch (error) { showMessage('loginMessage', `Нэвтэрч чадсангүй: ${error.message}`); } }
