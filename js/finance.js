...
async function syncToSheet(item) {
  ...
  await fetch(`https://sheets.googleapis.com/v4/spreadsheets/${settings.spreadsheetId}/values/Transactions!A:F:append?valueInputOption=USER_ENTERED&insertDataOption=INSERT_ROWS`, { method: 'POST', headers: { Authorization: `Bearer ${accessToken}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ values: [[item.date, item.type === 'income' ? 'Орлого' : 'Зарлага', item.category, item.amount, item.note, new Date().toISOString()]] }) });
}
byId('date').valueAsDate = new Date(); byId('monthFilter').value = new Date().toISOString().slice(0, 7); byId('spreadsheetId').value = settings.spreadsheetId || ''; updateCategories('income'); render();
