...
function validateTransaction(item) { return item.date && ['income', 'expense'].includes(item.type) && Number.isFinite(item.amount) && item.amount > 0 && item.amount <= 100000000000 && item.category.length <= 100 && item.note.length <= 500; }
...
async function loadTransactions() {
  ...
  try {
    const snapshot = await getDocs(query(transactionCollection(), orderBy('date', 'desc')));
    transactions = snapshot.docs.map((item) => ({ ...item.data(), id: item.id })); render(); setStatus('Firestore-тэй холбогдсон', true); showMessage('sheetMessage', 'Өгөгдөл шинэчлэгдлээ.');
...
async function saveTransaction(item, messageId = 'formMessage') {
  ...
  try {
    await runTransaction(db, async (transaction) => { const existing = await transaction.get(reference); if (existing.exists()) throw new Error('Энэ гүйлгээ аль хэдийн бүртгэгдсэн байна.'); transaction.set(reference, { ...item, userId: currentUser.uid, createdAt: serverTimestamp() }); });
    await loadTransactions(); showMessage(messageId, 'Амжилттай хадгалагдлаа.'); return true;
  } catch (error) { showMessage(messageId, `Хадгалсангүй: ${error.message}`); return false; }
}

async function updateTransaction(item) {
  if (!currentUser || !validateTransaction(item)) throw new Error('Invalid transaction.');
...
async function deleteTransaction(id) {
  if (!currentUser || !window.confirm('Энэ гүйлгээг устгах уу?')) return;
  try { await deleteDoc(transactionReference(id)); await loadTransactions(); showMessage('sheetMessage', 'Гүйлгээ устгагдлаа.'); }
  catch (error) { showMessage('sheetMessage', `Устгасангүй: ${error.message}`); }
}

async function clearTransactions() {
  if (!currentUser || !transactions.length || !window.confirm('Бүх гүйлгээг устгах уу?')) return;
  try { const snapshot = await getDocs(transactionCollection()); const batch = writeBatch(db); snapshot.docs.forEach((item) => batch.delete(item.ref)); await batch.commit(); await loadTransactions(); showMessage('sheetMessage', 'Бүх гүйлгээ устгагдлаа.'); }
  catch (error) { showMessage('sheetMessage', `Цэвэрлэсэнгүй: ${error.message}`); }
}
...
async function saveReceipt() { const total = receiptItems.reduce((sum, item) => sum + item.price * item.quantity, 0); if (!total) { showMessage('receiptMessage', 'Эхлээд баримтад бараа нэмнэ үү.'); return; } const item = { id: String(Date.now()), type: 'expense', amount: total, category: 'Хоол хүнс', date: byId('date').value, note: `Баримт: ${receiptItems.map((receiptItem) => receiptItem.name).join(', ')}` }; if (await saveTransaction(item, 'receiptMessage')) { receiptItems = []; renderReceipt(); showMessage('receiptMessage', 'Баримт зарлагаар хадгалагдлаа.'); } }

async function signIn() { if (!firebaseConfigured) { showMessage('loginMessage', 'Эхлээд firebase.js дотор Firebase config-оо оруулна уу.'); return; } try { await signInWithPopup(auth, googleProvider); } catch (error) { showMessage('loginMessage', `Нэвтэрч чадсангүй: ${error.message}`); } }
...
function hideApp() { currentUser = null; transactions = []; receiptItems = []; render(); renderReceipt(); byId('financeApp').classList.add('hidden'); byId('loginScreen').classList.remove('hidden'); setStatus('Firestore-тэй холбогдоогүй'); }

byId('loginButton').addEventListener('click', signIn); byId('signOutButton').addEventListener('click', logOut); byId('refreshButton').addEventListener('click', loadTransactions); byId('monthFilter').addEventListener('change', render); byId('clearButton').addEventListener('click', clearTransactions);
...
byId('transactionForm').addEventListener('submit', async (event) => { event.preventDefault(); const item = { id: crypto.randomUUID(), type: byId('transactionType').value, amount: Number(byId('amount').value), category: byId('category').value, date: byId('date').value, note: byId('note').value.trim() }; if (await saveTransaction(item)) { event.target.reset(); byId('date').valueAsDate = new Date(); updateCategories('income'); } });
